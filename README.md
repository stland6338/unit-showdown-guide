...
5. YouTube サムネイル画像・チャンネルアイコンは元配信/チャンネルへの**リンクの一部として**のみ使用し、単体で掲載しない
6. 公式ツイート・公式サイト文面の逐語転載をしない（事実の要約のみ。出典リンクは明記）。ただし **X公式埋め込みウィジェット経由の表示は転載に該当せず可**（コンテンツはX側が配信し、削除に自動追従するため）
